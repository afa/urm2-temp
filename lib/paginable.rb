 ...
 module Controller
  ...
  def paginate(parm = {})
   if parm.has_key?(:derid) && parm[:derid].is_a?(Hash)
    ...
    render_to_string :text => "<div class=\"pagination\">#{f_col.empty? ? "" : f_col.map{|i| link_to i.to_s, url_for(params.merge(:page => i))} }#{fm_col.size > options[:undotted_count] ? '...' : fm_col.map{|i| link_to i.to_s, url_for(params.merge(:page => i))}}</div>".html_safe
   else
