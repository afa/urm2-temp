 ...
 module ClassMethods
  ...
  EXPORTABLE_FIELDS = {
   :csv => {
    ...
    :sold_orders => [[:web, "Интернет портал"], [:0, "Код заказа"], [:0, "Номер заказа"], [:0, "ROHS"], [:0, "Наименование"], [:0, "Применение"], [:0, "Производитель"], [:0, "Корпус"], [:0, "Кол-во"], [:0, "Цена"], [:0, "Валюта"], [:0, "Сумма"], [:0, "Общая скидка в %"], [:0, "Скидка по позиции, %"], [:0, "Дата продажи"], [:0, "Склад отгрузки"], [:0, "Документ"], [:0, "Оф. номер накладной"], [:0, "ГТД"], [:0, "Страна происхождения"], [:0, "Код страны"], [:0, "БИН"], [:0, "Код номенклатуры"], [:0, "Покупатель"], [:0, "Контактное лицо"], [:0, "Лот закупки клиента"], [:0, "Место"]]
   }
  }
