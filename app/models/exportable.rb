...
module Exportable
 ...
 module ClassMethods
  ...
  EXPORTABLE_FIELDS = {
   ...
   :client_lines => {:types => [[:invoice_date, "Дата накладной"], [:invoice_num, "Номер накладной"], [:sales_id, "Номер заказа"], [:item_name, "Наименование"], [:producer, "Производитель"], [:rohs, "ROHS"],[:package, "Корпус"], [:qty, "Количество"], [:sales_price, "Цена"], [:amount, "Сумма с НДС"], [:currency, "Валюта"]]}
   }
