...
#  end
  ...
  def create_axapta_account
   ...
   accounts << Account.create(Axapta.user_info(self.ext_hash).inject({}){|r, a| r.merge(Account.axapta_renames[a[0]].nil? ? {a[0] => a[1]}: {Account.axapta_renames[a[0]] => a[1]}) }.delete_if{|k, v| not Account.axapta_attributes.include?(k.to_s) }.merge({:axapta_hash => self.ext_hash}))
   #ext_hash = nil
  end
