class CartsController < ApplicationController
 respond_to :js, :html, :json
  ...
  def destroy
   ...
   respond_with do |format|
    format.js { render :layout => false }
    format.json { render :json => {:carts_table => escape_javascript(render_string(:partial => "carts/cart_table", :locals => {:cart => @carts})), :old => @old, :new => @new, :carts_empty => @carts.empty?} }
   end
  end
