...
class MainController < ApplicationController
 ...
 skip_before_filter :check_account_cur, :only => [:index]
  ...
  def search
   ...
   if @search.query_string.length > 3 && !@items.detect{|i| i.name == @search.query_string }
   ...
   flash[:warn] = t "errors.search.empty" if @items.empty?
   flash[:warn] = t "errors.search.ambiq" if @items.ambiq
  end
