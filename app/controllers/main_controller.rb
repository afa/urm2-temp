 ...
 before_filter :check_account, :only => [:search, :extended, :dms]
  ...
  def mass_dms
   ...
   hsh = @items.inject({}) do |r, item|
    i = WebUtils.escape_name("#{item["item_name"]}_#{item["item_brend"]}_#{item["rohs"]}", "item_")
    unless r.has_key?(i)
     r[i] = []
    end
    r[i] << item
   ...
   dat = {}
   hsh.keys.each do |k|
    dat[k] = render_to_string :partial => "main/dms_block.html", :locals => {:items => hsh[k], :after => 0}
   end
   respond_with do |format|
