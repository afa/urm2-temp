 ...
 before_filter :get_filter
  ...
  def track
   respond_with do |format|
    format.json do
     p "---tr", params[:id]
     render :json => {:rendered => render_to_string(:partial => "orders/track.html.haml", :locals => {:tracks => Axapta.sales_tracking(:sales_id => params[:id])})}
    end
   end
  end
  ...
  def lines_track
   respond_with do |format|
    format.json do
     p "---ltr", params[:id]
     render :json => {:rendered => render_to_string(:partial => "orders/lines_track.html.haml", :locals => {:lines_tracks => Axapta.sales_tracking(:line_id => params[:id])})}
    end
   end
  end
 ...
 protected
  def get_filter
   ...
   if params[:filter]
    ...
    Setting.set_all(Hash[params[:filter].select{|k, v| %w(reservation_end only_my official_number sales_id this_sales_origin).include?(k) }.map{|k, v| ["order.#{k}", v] }])
   end
   @page = params[:page] || 1
  end
end
