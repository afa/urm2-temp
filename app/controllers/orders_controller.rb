class OrdersController < ApplicationController
 ...
 before_filter :get_filter
  ...
  def export_client_lines
   respond_with do |format|
    format.csv do
     send_data CartItem.export(:csv, :open_sales_lines, Axapta.sales_lines_paged(@page, @filter_hash.merge(:only_open => true)).items), :type => "application/csv", :disposition => :attachment
    end
   end
